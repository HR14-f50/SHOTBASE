...
`includes/theme_palette.php` の `profileThemes()` がテーマ一覧です。`white` が新規ユーザーの初期テーマで、白〜薄いグレーの背景と透明感のあるアクセントを使います。テーマを追加する場合は、次の配列にキー・表示名・アクセント色・淡い背景色を追加します。
...
カラーコードは6桁の16進数で指定します。`accent` はリンク・ボタン・タグの通常色、`soft` は背景色です。初期値を変更する場合は `white` の値を編集し、新規インストールでは `migrations/20260920_light_theme.sql` まで適用します。
