...
`includes/theme_palette.php` の `profileThemes()` がテーマ一覧です。現在はホワイト、プロ野球12球団、パープル、ピンク、ダークモードの16種類です。`white` が新規ユーザーの初期テーマで、既存ユーザーも `migrations/20260920_reset_theme_colors.sql` でホワイトへ移行します。テーマを追加する場合は、配列にキー・表示名・アクセント色・淡い背景色を追加します。
...
カラーコードは6桁の16進数で指定します。`accent` はリンク・ボタン・タグの通常色、`soft` は背景色です。ダークモードのキーは `dark` で、画面全体を暗色に切り替えるCSSも適用されます。
...
プロフィール画面・プロジェクト画面のプロフィールタグ一覧は表示しません。プロフィールへ移動する場合は、サイドバーのアイコンをクリックします。
