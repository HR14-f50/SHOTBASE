...
$stmt = db()->prepare('SELECT t.*, COUNT(DISTINCT ph.id) AS use_count FROM tags t JOIN photo_tags pt ON pt.tag_id = t.id JOIN photos ph ON ph.id = pt.photo_id WHERE ph.project_id = ? AND ph.user_id = ? AND ph.visibility = "public" GROUP BY t.id ORDER BY use_count DESC, t.name');
